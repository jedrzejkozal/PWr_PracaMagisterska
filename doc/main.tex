...
Aby zapewnić najlepsze osiągi porównywanych algorytmów z uwzględnieniem ograniczonej mocy obliczeniowej dokonano ręcznego doboru hiperparametrów modeli. Modele uczono przez 20 epok porównując osiągane wyniki. Do dalszych testów wybrano modele osiągające największe accuracy na zbiorze testowym. 

\begin{figure}
\centering
	\includegraphics[width=0.5\textwidth]{img/grid_search.png}
	\caption{Porównanie Grid Search i Random Search.} Źródło: https:\/\/i.stack.imgur.com\/cIDuR.png
	\label{fig:hyperparams}
\end{figure}

Inne dostępne rozwiązania problemu doboru hiperametrów to wykorzystanie algorytmów grid search lub random search. Grid search nie jest zalecaną metodą doboru hiperparametrów. Często inne hiperparametry mają większy bądź mniejszy wpływ na wyniki procesu uczenia. Dodatkową wadą jest możliwość występowania maksimum optymalizowanej metryki pomiędzy punktami tworzonej siatki, jak to zostało przedstawione na rys. \ref{fig:hyperparams}. Wadą obu jest eksponencjalnie rosnąca liczba prób dla kolejnych hiperparametrów.

Z tego powodu zdecydowano podzielić proces doboru hiperparametrów na kolejne kroki, które wykonano w celu doboru najlepszego modelu. Zostały one przedstawione w (tutaj wkleić algorytm).

1. Dobór hiprametrów mających największy wpływ na proces uczenia i.e. learning rate i współczynniki reugraryzacji l1 i l2 warstw Dense
2. Dobór liczby neuronów w warstwach Dense
3. Dobór liczby warstw odpowiedzialnych za ekstrakcję cech (warstwy ReNet, zmodyfikowana ReNet oraz konwolucyjna)
4. Dobór liczby neuronów w warstwach odpowiedzialnych za ekstrakcję cech.
5. Dobór metod i współczynników reguralizacji warstwach odpowiedzialnych za ekstrakcję cech.

Warto zwrócić uwagę, że w punkcie 2. przyjęto tą samą liczbę warstw Dense dla wszystkich modeli - 2. W celu porównania architektury ReNet, zmodyfikowanej ReNet oraz konwolucyjnej i ich zdolności do ekstrachowania odpowiednich cech z obrazów modele powinny dysponować takimi samymi warstwami dokonującymi klasyfikacji na podstawie nauczonej reprezentacji.

W przypadku parametrów odpowiadających za liczność neuronów w warstawach porówywano osiągane wyniki dla kolejnych potęg 2 z zakresu $[32, 4096]$. Gdy powyżej pewnej wartości sieć osiągała znacząco gorsze wyniki, przerywano proces poszukiwania i przechodzono do następnego hiperparametru badanego modelu. W przypadku parametrów odpowiedzialnych za proces uczenia wybierano parametry z zakresu $[10^{-3}, 10^{-8}]$ w skali logarytmicznej, ponownie przerywając poszukiwania, jeśli powyżej pewnego zakresu wyniki nie były poprawiane.

Opisana metoda pozwala na zmniejszenie nakładu obliczeniowego potrzebnego do wyboru modelu, jednocześnie pozwala na ustrukturyzowany wybór modelu. Otrzymany model nie będzie optymalnym rozwiązaniem w analizowanej przestrzeni, ale będzie wystarczająco dobry do przeprowadzenia porównania testów. 
