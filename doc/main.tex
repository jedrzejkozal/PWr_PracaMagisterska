...
\begin{tikzpicture}[auto,node distance=1.5cm]
	\tikzstyle{node}=[circle,draw]
		...
		\node (xt_1) [below=1.5cm of ht_1] {$x_{t-1}$};
			...
			\draw[->, bend left=30, red] (ot_1) edge node {$\frac{\partial \mathcal{E}_{t}}{\partial \bb{x}_{t-1}}$} (ht_1);
			\draw[->, bend left=30, red] (ht_1) edge node {$\frac{\partial \bb{h}_{t-1}}{\partial \bb{h}_{t-2}}$} (h);
		...
		\node (xt) [below=1.5cm of ht] {$x_{t}$};
			...
			\draw[->, bend left=30, red] (ot) edge node {$\frac{\partial \mathcal{E}_{t}}{\partial \bb{x}_{t}}$} (ht);
			\draw[->, bend left=30, red] (ht) edge node {$\frac{\partial \bb{h}_{t}}{\partial \bb{h}_{t-1}}$} (ht_1);
		...
		\node (xt1) [below=1.5cm of ht1] {$x_{t+1}$};
			...
			\draw[->, bend left=30, red] (ot1) edge node {$\frac{\partial \mathcal{E}_{t}}{\partial \bb{x}_{t+1}}$} (ht1);
			\draw[->, bend left=30, red] (ht1) edge node {$\frac{\partial \bb{h}_{t+1}}{\partial \bb{h}_{t}}$} (ht);
		\node (h_end) [right=2.0cm of ht1] {$ $};
			\draw[->] (ht1) edge node {$ $} (h_end);
			\draw[->, bend left=30, red] (h_end) edge node {$\frac{\partial \bb{h}_{t+2}}{\partial \bb{h}_{t+1}}$} (ht1);
\end{tikzpicture}
\caption{Schemat działania propagacji wstecznej w czasie.} Czarnym kolorem zaznaczono przepływ informacji przy obliczaniu aktywacji sieci (foreward pass). Czerwonym kolorem zaznaczono przepływ informacji przy zastosowaniu wstecznej propagacji błędu.
	\label{fig:rnn_backprop}
\end{figure}

Błąd propagowany wstecz dla każdego kroku $\mathcal{E}_t$ można zapisać jako wartość funkcji strat od aktywacji w danym kroku:
...
\begin{equation}
	\frac{\partial \mathcal{E}_t}{\partial \theta} = \sum_{1 \leq k \leq t} (\frac{\partial \mathcal{E}_t}{\partial \bb{h}_t} \frac{\partial \bb{h}_t}{\partial \bb{h}_k} \frac{\partial \bb{h}_k}{\partial \theta})
\end{equation}

Intuicyjnie suma w powyższym wzorze określa kolejno wpływ wszystkich stanów sieci $k$ na aktualną wartość aktywacji w czasie $t$. Wyrażenie $\frac{\partial \bb{h}_t}{\partial \bb{h}_k}$ można zapisać stosując regułę łańcucha jako:

\begin{equation}
	\frac{\partial \bb{h}_t}{\partial \bb{h}_k} = \prod_{t \geq i > k} \frac{\partial \bb{h}_i}{\partial \bb{h}_{i-1}} = \prod_{t \geq i > k} \bb{W}^{T}_{rec} diag(\sigma'(\bb{h}_{i-1}))
\end{equation}

rys. \ref{fig:rnn_backprop}.
