...
\begin{tabular}{ |c|c|c|c| } 
 ...
 \hline
 \thead{mnist} & 0.0045 & 0.0628 & (0.0675, 0.0580) \\ 
 \hline
 \thead{cifar10} & 0.1235 & 0.3787 & (0.3882, 0.3691) \\ 
 \hline
 \thead{svhn} & 0.0238 & 0.5602 & (0.5662, 0.5541) \\ 
 \hline
\end{tabular}
\end{table}

Wynik reprodukcji został przedstawiony w tabeli \ref{tab:reproduction}. Wyraźnie widać, że wyniki uzyskane przez twórców architektury ReNet są lepsze niż wyniki uzyskane w tej pracy. Żaden z wcześniej uzyskanych wyników nie mieści się przedziale ufności błędu obecnej implementacji.

Szczególnie niepokojące mogą być wartości błędu dla zbiorów cifar10 oraz svhn. W przypadku drugiego datasetu jest to związane z przerwaniem obliczeń po 5 epokach ze względu na brak pamięci. Model stosowany przy obecnej implementacji dla svhn był kosztowny pod względem pamięci i jego trenowanie przebiegało niezwykle wolno - ponad 2 godziny na epokę uczenia. Zaobserwowano znaczący spadek czasu uczenia sieci przy zastosowaniu RMSprop jak algorytmu optymalizacji, jednakże jego zastosowanie w tym przypadku nie byłoby wierną reprodukcją wcześniejszych badań. Przyczyną znacząco gorszych wyników uzyskanych dla cifar10 i svhn może być także dobór niewystarczająco dokładnych hiperparametrów sieci. Twórcy ReNet nie podali hiperparametrów stosowanych w ich eksperymentach, co znacząco utrudnia reprodukcję.

W związku z uzyskanymi rezultatami nasuwa się pytanie czy przygotowana implementacja sieci ReNet jest błędna? Warto zwrócić uwagę na bardzo zbliżone wyniki uzyskane dla datasetu mnist. Dodatkowo wyżej wymienione powody słabych wyników nie muszą wskazywać na wadliwość implementacji. W celu dodatkowej weryfikacji przed przystąpieniem do dalszej pracy przygotowano test poprawności implementacji ReNet z wykorzystaniem pewnej własności. Przy odpowiednio dobranych wartościach wag i funkcji aktywacji sieć LSTM może w każdym kroku czasowym zwracać aktualną wartość podaną na wejście. Dla wcześniej przyjętej parametryzacji LSTM można przyjąć wartości wag:
...
Aby zapewnić najlepsze osiągi porównywanych algorytmów z uwzględnieniem ograniczonej mocy obliczeniowej dokonano doboru wybranych hiperparametrów modeli. Do zbioru optymalizowanych parametrów dla każdego zbioru uczącego należały:
