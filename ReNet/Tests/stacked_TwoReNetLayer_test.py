...
class TestStackedTwoReNetLayer(object):
    ...
    def sut(self):
        ...
        sut = Model(inputs=input, outputs=second_output)
        return sut
    ...
    def test_call_output_shape_is_valid(self, sut):
        ...
        assert result_shape == [self.img_width // (2*self.w_p),
                self.img_height // (2*self.h_p),
                2*self.hidden_size]
