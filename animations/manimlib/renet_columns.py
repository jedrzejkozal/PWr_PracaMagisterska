...
class ReNetColumns(ThreeDScene):

    def setup(self):
        self.camera.set_frame_width(self.camera.get_frame_width()+10)
        self.camera.set_frame_height(self.camera.get_frame_height()+5)
    ...
    def construct(self):
        ...
        animations = self.simultaneous_movement_animations(activation_patch2, PhaseFlow, lambda x: np.array([3,0,0]))
        self.play(*animations)
        animations = self.simultaneous_movement_animations(activation_patch1, PhaseFlow, lambda x: np.array([7,0,0])) + self.simultaneous_movement_animations(activation_patch2, PhaseFlow, lambda x: np.array([0,-4.5,0]))
        self.play(*animations)
        animations = self.simultaneous_movement_animations(activation_patch0, PhaseFlow, lambda x: np.array([11,0,0])) + self.simultaneous_movement_animations(activation_patch1, PhaseFlow, lambda x: np.array([0,-3.5,0]))
        self.play(*animations)
        animations = self.simultaneous_movement_animations(activation_patch0, PhaseFlow, lambda x: np.array([0,-2.5,0]))
        self.play(*animations)
    ...
    def play_img_creation(self, img):
        for column in img:
            animations = self.simultaneous_animations(column, ShowCreation)
            self.play(*animations)
